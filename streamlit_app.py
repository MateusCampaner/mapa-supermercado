...
def adicionar_item():
    ...
    if item:
        ...
        # Atualizar a imagem após adicionar um item
        atualizar_imagem()

def limpar_lista():
    st.session_state.itens.clear()  
    st.session_state.selected_image = "mapa-sem-rota.png"  
    # Não precisa de rerun aqui, a interface vai se atualizar automaticamente

def atualizar_imagem():
    # Variável para contar quantos itens foram ticados
    itens_ticados = sum(st.sidebar.checkbox(item, value=item in st.session_state.itens) for item in st.session_state.itens)
...
# Exibe checkboxes para os itens na lista
for item in st.session_state.itens:
    if st.sidebar.checkbox(item, value=item in st.session_state.itens):
        pass  # A contagem de itens ticados é feita na função atualizar_imagem
